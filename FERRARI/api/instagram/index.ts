...
// Mock data for when API credentials are not configured
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const mediaResponse = await fetch(
      `https://graph.instagram.com/${userId}/media?fields=id,caption,media_type,media_url,permalink,thumbnail_url,timestamp&limit=6&access_token=${accessToken}`
    );
    ...
    const posts = await Promise.all(
      mediaData.data.slice(0, 6).map(async (post: any) => {
        try {
